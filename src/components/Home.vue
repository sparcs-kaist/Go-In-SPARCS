        ...
        <v-list three-line>
          <template v-for="(item, index) in items">
            ...
            >
              ...
              <v-list-item-action>
                <v-list-item-action-text v-html="item.info"></v-list-item-action-text>
                <v-icon
                        color="red lighten-1"
                        class="mt-4"
...
export default {
  ...
  mounted(){
    this.$socket = io(`${window.location.protocol}//${window.location.host}`, {query: {token: this.getAccessToken}, secure: true});
